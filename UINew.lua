...
local requestsDisabled = true --getgenv and getgenv().DISABLE_RayFun_REQUESTS
local InterfaceBuild = '3K3W'
local Release = "Build 1.3"
local RayFunFolder = "RayFun "
local ConfigurationFolder = RayFunFolder.."/Configurations"
local ConfigurationExtension = ".rfld"
local settingsTable = {
	General = {
		-- if needs be in order just make getSetting(name)
		RayFunOpen = {Type = 'bind', Value = 'R', Name = 'RayFun  Keybind'},
		-- buildwarnings
		-- RayFunprompts
...
local overriddenSettings: { [string]: any } = {} -- For example, overriddenSettings["System.RayFunOpen"] = "J"
...
local function loadSettings()
	...
	local success, result =	pcall(function()
		task.spawn(function()
			if isfolder and isfolder(RayFunFolder) then
				if isfile and isfile(RayFunFolder..'/settings'..ConfigurationExtension) then
					file = readfile(RayFunFolder..'/settings'..ConfigurationExtension)
				end
			end
			...
			if useStudio then
				file = [[
		{"General":{"RayFunOpen":{"Value":"K","Type":"bind","Name":"RayFun  Keybind","Element":{"HoldToInteract":false,"Ext":true,"Name":"RayFun  Keybind","Set":null,"CallOnChange":true,"Callback":null,"CurrentKeybind":"K"}}},"System":{"usageAnalytics":{"Value":false,"Type":"toggle","Name":"Anonymised Analytics","Element":{"Ext":true,"Name":"Anonymised Analytics","Set":null,"CurrentValue":false,"Callback":null}}}}
	]]
			end
	...
	if not success then 
		if writefile then
			warn('RayFun  had an issue accessing configuration saving capability.')
		end
	end
end
...
local RayFunLibrary = {
	...
	Theme = {
		...
		Purpleset = {
			...
			Topbar = Color3.fromRGB(25, 0, 51),
			Shadow = Color3.fromRGB(25, 0, 51),

			NotificationBackground = Color3.fromRGB(25, 0, 51),
			NotificationActionsBackground = Color3.fromRGB(230, 230, 230),

			TabBackground = Color3.fromRGB(25, 0, 51),
			TabStroke = Color3.fromRGB(25, 0, 51),
			TabBackgroundSelected = Color3.fromRGB(210, 210, 210),
			TabTextColor = Color3.fromRGB(240, 240, 240),
			SelectedTabTextColor = Color3.fromRGB(50, 50, 50),

			ElementBackground = Color3.fromRGB(25, 0, 51),
			ElementBackgroundHover = Color3.fromRGB(25, 0, 51),
			SecondaryElementBackground = Color3.fromRGB(25, 0, 51),
			ElementStroke = Color3.fromRGB(50, 50, 50),
			SecondaryElementStroke = Color3.fromRGB(25, 40, 51),
		...
		AMOLEDPurple = {
			...
			TabStroke = Color3.fromRGB(0, 0, 0),
			TabBackgroundSelected = Color3.fromRGB(210, 210, 210),
			TabTextColor = Color3.fromRGB(75, 0, 130),
			SelectedTabTextColor = Color3.fromRGB(50, 50, 50),
...
local RayFun = useStudio and script.Parent:FindFirstChild('RayFun') or game:GetObjects("rbxassetid://10804731440")[1]
local buildAttempts = 0
local correctBuild = false
local warned
local globalLoaded
local RayFunDestroyed = false -- True when RayFunLibrary:Destroy() is called

repeat
	if RayFun:FindFirstChild('Build') and RayFun.Build.Value == InterfaceBuild then
		correctBuild = true
		break
	end
	...
	if not warned then
		warn('RayFun | Build Mismatch')
		print('RayFun may encounter issues as you are running an incompatible interface version ('.. ((RayFun:FindFirstChild('Build') and RayFun.Build.Value) or 'No Build') ..').\n\nThis version of RayFun is intended for interface build '..InterfaceBuild..'.')
		warned = true
	end

	toDestroy, RayFun = RayFun, useStudio and script.Parent:FindFirstChild('RayFun') or game:GetObjects("rbxassetid://10804731440")[1]
	if toDestroy and not useStudio then toDestroy:Destroy() end

	buildAttempts = buildAttempts + 1
until buildAttempts >= 2

RayFun.Enabled = false

if gethui then
	RayFun.Parent = gethui()
elseif syn and syn.protect_gui then 
	syn.protect_gui(RayFun)
	RayFun.Parent = CoreGui
elseif not useStudio and CoreGui:FindFirstChild("RobloxGui") then
	RayFun.Parent = CoreGui:FindFirstChild("RobloxGui")
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == RayFun.Name and Interface ~= RayFun then
			Interface.Enabled = false
			Interface.Name = "RayFun-Old"
		end
	end
elseif not useStudio then
...
if RayFun.AbsoluteSize.X < minSize.X and RayFun.AbsoluteSize.Y < minSize.Y then
...
local Main = RayFun.Main
local MPrompt = RayFun:FindFirstChild('Prompt')
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList
local dragBar = RayFun:FindFirstChild('Drag')
...
RayFun.DisplayOrder = 100
LoadingFrame.Version.Text = Release

-- Thanks to Latte Softworks for the Lucide integration for Roblox
local Icons = useStudio and require(script.Parent.icons) or loadWithTimeout('https://raw.githubusercontent.com/SiriusSoftwareLtd/RayFun/refs/heads/main/icons.lua')
-- Variables
...
local Notifications = RayFun.Notifications

local SelectedTheme = RayFunLibrary.Theme.Default

local function ChangeTheme(Theme)
	if typeof(Theme) == 'string' then
		SelectedTheme = RayFunLibrary.Theme[Theme]
	elseif typeof(Theme) == 'table' then
		SelectedTheme = Theme
	end

	RayFun.Main.BackgroundColor3 = SelectedTheme.Background
	RayFun.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	RayFun.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	RayFun.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow

	RayFun.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
	RayFun.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
	RayFun.Main.Topbar.Search.ImageColor3 = SelectedTheme.TextColor
	if Topbar:FindFirstChild('Settings') then
		RayFun.Main.Topbar.Settings.ImageColor3 = SelectedTheme.TextColor
		RayFun.Main.Topbar.Divider.BackgroundColor3 = SelectedTheme.ElementStroke
	end
	...
	for _, text in ipairs(RayFun:GetDescendants()) do
...
local function getAssetUri(id: any): string
	...
	elseif type(id) == "string" and not Icons then
		warn("RayFun | Cannot use Lucide icons as icons library is not loaded")
	else
		warn("RayFun | The icon argument must either be an icon ID (number) or a Lucide icon name (string)")
	end
	return assetUri
end
...
local function LoadConfiguration(Configuration)
	...
	if not success then warn('RayFun  had an issue decoding the configuration file, please try delete the file and reopen RayFun .') return end

	-- Iterate through current UI elements' flags
	for FlagName, Flag in pairs(RayFunLibrary.Flags) do
		...
		if (typeof(FlagValue) == 'boolean' and FlagValue == false) or FlagValue then
			...
			warn("RayFun  | Unable to find '"..FlagName.. "' in the save file.")
			print("The error above may not be an issue if new elements have been added or not been set values.")
			--RayFunLibrary:Notify({Title = "RayFun Flags", Content = "RayFun was unable to find '"..FlagName.. "' in the save file. Check sirius.menu/discord for help.", Image = 3944688398})
		end
	end
...
local function SaveConfiguration()
	...
	local Data = {}
	for i, v in pairs(RayFunLibrary.Flags) do
...
function RayFunLibrary:Notify(data) -- action e.g open messages
	task.spawn(function()
		...
		if data.Actions then
			warn('RayFun  | Not seeing your actions in notifications?')
...
local function Hide(notify: boolean?)
	...
	if notify then
		if useMobilePrompt then 
			RayFunLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping 'Show'.", Duration = 7, Image = 4400697855})
		else
			RayFunLibrary:Notify({Title = "Interface Hidden", Content = `The interface has been hidden, you can unhide the interface by tapping {getSetting("General", "RayFunOpen")}.`, Duration = 7, Image = 4400697855})
		end
	end
...
local function saveSettings() -- Save settings to config file
	...
	if success then
		...
		if writefile then
			writefile(RayFunFolder..'/settings'..ConfigurationExtension, encoded)
		end
	end
end
...
local function createSettings(window)
	...
	local newTab = window:CreateTab('RayFun  Settings', 0, true)

	if TabList['RayFun  Settings'] then
		TabList['RayFun  Settings'].LayoutOrder = 1000
	end

	if Elements['RayFun  Settings'] then
		Elements['RayFun  Settings'].LayoutOrder = 1000
	end
...
function RayFunLibrary:CreateWindow(Settings)
	print('creating window')
	if RayFun:FindFirstChild('Loading') then
		if getgenv and not getgenv().RayFunCached then
			RayFun.Enabled = true
			RayFun.Loading.Visible = false
	...
	if getgenv then getgenv().RayFunCached = true end

	if not correctBuild and not Settings.DisableBuildWarnings then
		task.delay(3, 
			function() 
				RayFunLibrary:Notify({Title = 'Build Mismatch', Content = 'RayFun  may encounter issues as you are running an incompatible interface version ('.. ((RayFun:FindFirstChild('Build') and RayFun.Build.Value) or 'No Build') ..').\n\nThis version of RayFun  is intended for interface build '..InterfaceBuild..'.\n\nTry rejoining and then run the script twice.', Image = 4335487866, Duration = 15})		
			end)
	end
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "RayFun "
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "Interface"

	if Settings.LoadingTitle ~= "RayFun " then
		LoadingFrame.Version.Text = "RayFun "
	end
	...
	if not Settings.DisableRayFunPrompts then
		task.spawn(function()
			while true do
				task.wait(math.random(180, 600))
				RayFunLibrary:Notify({
					Title = "RayFun  Interface",
					Content = "I hope your enjoying RayFun !",
					Duration = 7,
					Image = 4370033185,
				})
			end
	...
	if Settings.Discord and Settings.Discord.Enabled and not useStudio then
		if isfolder and not isfolder(RayFunFolder.."/Discord Invites") then
			makefolder(RayFunFolder.."/Discord Invites")
		end

		if isfile and not isfile(RayFunFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if requestFunc then
				pcall(function()
					requestFunc({
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				writefile(RayFunFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"RayFun  RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		end
	end
	...
	if (Settings.KeySystem) then
		...
		if isfolder and not isfolder(RayFunFolder.."/Key System") then
			makefolder(RayFunFolder.."/Key System")
		end
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("RayFun  | "..Key.." Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with RayFun  specific development.')
				end
			end
		end
		...
		if isfile and isfile(RayFunFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			for _, MKey in ipairs(Settings.KeySettings.Key) do
				if string.find(readfile(RayFunFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension), MKey) then
					Passthrough = true
				end
			end
		end
		...
		if not Passthrough then
			...
			KeyMain.Subtitle.Text = Settings.KeySettings.Subtitle or "RayFun  Key System"
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				...
				if KeyFound then 
					...
					RayFunLibrary:Notify({Title = "Key System", Content = "The key for this script is correct.", Image = 110139098421372})
					
					if Settings.KeySettings.SaveKey then
						if writefile then
							writefile(RayFunFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, FoundKey)
						end
						RayFunLibrary:Notify({Title = "Key System", Content = "The key for this script has been saved successfully.", Image = 110139098421372})
					end
				else
	...
	function Window:CreateTab(Name, Image, Ext)
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				-- Prevents animation from trying to play if the button's callback called RayFunLibrary:Destroy()
				if RayFunDestroyed then
					return
				end
				if not Success then
					...
					print("RayFun  | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with RayFun  specific development.')
					task.wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
			...
			ColorPicker.Interact.MouseButton1Down:Connect(function()
				...
				if not opened then
					...
					TweenService:Create(Main, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {ImageTransparency = SelectedTheme ~= RayFunLibrary.Theme.Default and 0.25 or 0.1}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
					RayFunLibrary.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
				end
			end
			...
			RayFun.Main:GetPropertyChangedSignal('BackgroundColor3'):Connect(function()
				for _, rgbinput in ipairs(ColorPicker.RGB:GetChildren()) do
					if rgbinput:IsA("Frame") then
						rgbinput.BackgroundColor3 = SelectedTheme.InputBackground
						rgbinput.UIStroke.Color = SelectedTheme.InputStroke
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				if not Success then
					...
					print("RayFun  | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with RayFun  specific development.')
					task.wait(0.5)
					Input.Title.Text = InputSettings.Name
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and InputSettings.Flag then
					RayFunLibrary.Flags[InputSettings.Flag] = InputSettings
				end
			end
		...
		function Tab:CreateDropdown(DropdownSettings)
				...
				for _, Option in ipairs(DropdownSettings.Options) do
					...
					DropdownOption.Interact.MouseButton1Click:Connect(function()
						...
						if not Success then
							...
							print("RayFun  | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with RayFun  specific development.')
							task.wait(0.5)
							Dropdown.Title.Text = DropdownSettings.Name
							TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
					RayFunLibrary.Flags[DropdownSettings.Flag] = DropdownSettings
				end
			end
		...
		function Tab:CreateKeybind(KeybindSettings)
				...
				elseif not KeybindSettings.CallOnChange and KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then -- Test
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("RayFun  | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with RayFun  specific development.')
							task.wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					RayFunLibrary.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			if SelectedTheme ~= RayFunLibrary.Theme.Default then
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("RayFun  | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with RayFun  specific development.')
					task.wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if not ToggleSettings.Ext then
				if Settings.ConfigurationSaving then
					if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
						RayFunLibrary.Flags[ToggleSettings.Flag] = ToggleSettings
					end
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
					...
					if SLDragging then
						...
						if SliderSettings.CurrentValue ~= NewValue then
							...
							if not Success then
								...
								print("RayFun  | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
								warn('Check docs.sirius.menu for help with RayFun  specific development.')
								task.wait(0.5)
								Slider.Title.Text = SliderSettings.Name
								TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					RayFunLibrary.Flags[SliderSettings.Flag] = SliderSettings
				end
			end
	...
	function Window.ModifyTheme(NewTheme)
		...
		if not success then
			RayFunLibrary:Notify({Title = 'Unable to Change Theme', Content = 'We are unable find a theme on file.', Image = 123987057699387})
		else
			RayFunLibrary:Notify({Title = 'Theme Changed', Content = 'Successfully changed theme to '..(typeof(NewTheme) == 'string' and NewTheme or 'Custom Theme')..'.', Image = 110139098421372})
		end
	end
	...
	if not success then warn('RayFun  had an issue creating settings.') end
...
function RayFunLibrary:SetVisibility(visibility: boolean)
	setVisibility(visibility, false)
end

function RayFunLibrary:IsVisible(): boolean
	return not Hidden
end

local hideHotkeyConnection -- Has to be initialized here since the connection is made later in the script
function RayFunLibrary:Destroy()
	RayFunDestroyed = true
...
if Topbar:FindFirstChild('Settings') then
	Topbar.Settings.MouseButton1Click:Connect(function()
		task.spawn(function()
			...
			Elements.UIPageLayout:JumpTo(Elements['RayFun  Settings'])
		end)
	end)
...
hideHotkeyConnection = UserInputService.InputBegan:Connect(function(input, processed)
	if (input.KeyCode == Enum.KeyCode[getSetting("General", "RayFunOpen")]) and not processed then
		if Debounce then return end
		if Hidden then
			Hidden = false
			Unhide()
...
function RayFunLibrary:LoadConfiguration()
	local config
	...
	if CEnabled then
		...
		local success, result = pcall(function()
			...
			if isfile then 
				...
				notified = true
				RayFunLibrary:Notify({Title = "RayFun Configurations", Content = "We couldn't enable Configuration Saving as you are not using software with filesystem support.", Image = 4384402990})
			end
		end)

		if success and loaded and not notified then
			RayFunLibrary:Notify({Title = "RayFun Configurations", Content = "The configuration file for this script has been loaded from a previous session.", Image = 4384403532})
		elseif not success and not notified then
			warn('RayFun Configurations Error | '..tostring(result))
			RayFunLibrary:Notify({Title = "RayFun Configurations", Content = "We've encountered an issue loading your configuration correctly.\n\nCheck the Developer Console for more information.", Image = 4384402990})
		end
	end
...
if useStudio then
	...
	--local Window = RayFunLibrary:CreateWindow({
	--	Name = "RayFun Example Window",
	--	LoadingTitle = "RayFun Interface Suite",
	--	Theme = 'Default',
	--	Icon = 0,
	...
	--		FileName = "Key", -- It is recommended to use something unique as other scripts using RayFun may overwrite your key file
	--		SaveKey = true, -- The user's key will be saved, but if you change the key, they will be unable to use your script
	--		GrabKeyFromSite = false, -- If this is true, set Key below to the RAW site you would like RayFun to get the key from
	...
	----RayFunLibrary:Notify({Title = "RayFun Interface", Content = "Welcome to RayFun. These - are the brand new notification design for RayFun, with custom sizing and RayFun calculated wait times.", Image = 4483362458})
	...
	--local thoptions = {}
	--for themename, theme in pairs(RayFunLibrary.Theme) do
...
task.delay(4, function()
	RayFunLibrary.LoadConfiguration()
